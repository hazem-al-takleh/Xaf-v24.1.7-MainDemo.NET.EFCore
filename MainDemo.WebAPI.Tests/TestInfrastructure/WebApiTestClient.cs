...
namespace MainDemo.WebAPI.TestInfrastructure {
    ...
    public class WebApiClient {
        ...
        public HttpRequestMessage SerializeToHttpRequest(HttpMethod method, string uri, Type type, object value) {
            var request = new HttpRequestMessage(method, uri);

            var json = JsonSerializer.Serialize(value, type, new JsonSerializerOptions {
        ...
        public async Task<JsonElement> RequestAsync(HttpRequestMessage request, bool preventAuthorization = false) {
            ...
            if(httpResponse.StatusCode == System.Net.HttpStatusCode.NoContent) {
                //throw new InvalidOperationException($"{request.Method} request has no JSON! Code {(int)httpResponse.StatusCode}, '{httpResponse.ReasonPhrase}'");
        ...
        public Task PutAsync<T>(string key, T value) where T : notnull // complete update
            => SendAsync(
                SerializeToHttpRequest(HttpMethod.Put, GetTypeUri<T>(key), value));

        public Task PatchAsync<T>(string key, T value) where T : notnull // partial update
            => SendAsync(
        ...
        public Task PutAsync(Type type, string key, object value) // complete update
            => SendAsync(
                SerializeToHttpRequest(HttpMethod.Put, GetTypeUri(type, key), type, value));

        public Task PatchAsync(Type type, string key, object value) // partial update
            => SendAsync(
